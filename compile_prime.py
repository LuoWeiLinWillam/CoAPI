...
def compile_cover_NPCNF(project_file_path, limit_time, output_file_path_ana, uca_open, qx_open, zm_open, iteration_limit, backtrack_mod, is_coherent, positive_input_file_path, negative_input_file_path, output_file_path_cpi):

	command = '/usr/bin/time -v ' + project_file_path +  ' -op=1 -of=' + output_file_path_ana
	if uca_open:
		command = command + ' -uca'
	elif qx_open:
		command = command + ' -qx'
		...
		command = command + ' -zm'
	command = command + ' -it=' + iteration_limit + ' -bt=' + backtrack_mod
	if is_coherent:
		command = command + ' -co'
	command = command + ' ' + positive_input_file_path + ' ' + negative_input_file_path + ' > ' + output_file_path_cpi
	# print command
...
def compile_all_NPCNF(project_file_path, limit_time, output_mod, input_file_path, output_file_path, is_PIn, backtrack_mod):
	
	command = '/usr/bin/time -v' + ' ' + project_file_path + ' -bt=' + backtrack_mod + ' -op=' + output_mod + ' ' + input_file_path + ' > ' + output_file_path
	# print command
...
def decode(input_file_path, is_PIn, output_mod):
	is_error = True
	output = []
...
def read_all_PI(input_file_path, output_mod):
	# all_PI = []
	num_all_temp = 0
	num_all = 0
	memory = 0
